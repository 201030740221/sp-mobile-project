module.exports =
    ...
    transformSkuData: (skuObj)->
        ...
        for sku in skuObj
            ...
            attributeKeyGroup = product_sku["attribute_key"].split(",")
    ...
    #初始化得到结果集
    initSKU : ->
        store = liteFlux.store("detail").getStore()
        skuData = @transformSkuData JSON.parse store.goodData.currentSkuJson
    ...
    renderSku: ->
        ...
        # 处理组合
        @initSKU()
        #console.log store.attribute_key
        #console.log @SKUResult
        return store.goodData.skuData.map (item)->
            # 如果属性不为空
            if item.value.length
                return self.renderSkuItem(item)
    # 获得对应的 SKU 组合
    getSelectSku: ->
        selectedIds = []
        store = liteFlux.store("detail").getStore()
        for key of store.attribute_key
            if store.attribute_key[key]
                selectedIds.push store.attribute_key[key]
    ...
    selectItem: (data,e)->
        if !(/choice-disabled/.test e.target.className)
            store = liteFlux.store("detail").getStore()
            len = Object.keys(store.attribute_key).length
            if store.attribute_key[data.attribute_id] ==  data.id
                store.attribute_key[data.attribute_id] = null
            else
                store.attribute_key[data.attribute_id] =  data.id
            # 保存 store
            liteFlux.store("detail").setStore(store)
    ...
    renderChildItem: (data)->
        ...
        data.map (child)->
            ...
            if store.attribute_key[child.attribute_id] ==  child.id
                checked = true
            ...
            # 存放所有与相邻属性组合
            attrIds = []
            store.goodData.skuData.map (res)->
                # 如果属性值不为空且不是当前属性组
            ...
            <ChoiceCheckbox.Item key={child.id} disabled={disabled} status={checked} onClick={self.selectItem.bind null,child}>
    ...
    renderSkuItem: (item)->
        ...
        item_name = item.name
        return (
            <Grid key={item.id} cols={24} className="label-list">
