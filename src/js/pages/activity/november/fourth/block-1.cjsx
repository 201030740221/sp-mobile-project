...
PageView = React.createClass
    ...
    renderRule: () ->
        ...
        <Alertable className="u-f12 u-color-yellow" showConfirmBtn={no} showCloseIcon={yes} contentElement={rule} margun={15}>抽奖规则</Alertable>
    hideAlert: () ->
        Action.hideAlert()
        Action.setResult null
    showAlert: () ->
    ...
    render: ->
        ...
        timesTextNode = ''
        if lottery.times > -1 and @isLogin()
            timesTextNode =
                <div className="u-color-white u-text-center">你目前拥有<span className="u-f14">{lottery.times}</span>次抽奖机会，大奖在等你哟！</div>
        <div className="lottery-marquee lottery-marquee-2 activity-november-4-block-1">
            <div className="activity-november-4-title single-row">
                <div className="activity-november-4-title-text">拼拼手气，免单大奖送到家</div>
            </div>
            {timesTextNode}
            <LotteryMarquee data={lottery.prizeList} begin={begin} end={end} times={lottery.times} result={result} start={@start} callback={@callback} isLogin={@isLogin()} login={@login} contentType="2" rule={rule} />
            {@renderResultAlertWithAddress()}
        </div>
